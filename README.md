...
- 想先理解定位：看“为什么要做这个仓库”与“核心特性”
- 想快速上手：看“快速开始”
- 想看结构设计：看“当前推荐目录结构”与“目标架构（演进方向）”
- 想看使用边界：看“安全说明”“常见问题”和“Roadmap”
...
这套规则系统不是一次性加载全部规则，而是采用**渐进式、可回退**的方式运行。核心口径只有一条：
...
这些内容属于未来演进方向，不是当前阶段的硬依赖。当前已完成的补充包括：
...
- 想直接在项目里使用：看“方式一”
- 想把本仓库当规则源复用：看“方式二”
- 想批量同步 `.claude` 与 settings 模板：看“方式三”
...
如果你希望直接复用本仓库，推荐按“两步走”理解这一部分：先同步完整 `.claude` 目录，再按需叠加场景化 settings 模板。
...
> 注意：不是所有文件都需要一开始就存在。对于可选偏好文件，如果文件不存在、不可读，或当前环境不支持加载，可以直接忽略，不应阻断任务执行。

---

## 目标架构（演进方向）

如果把上一节看作“当前可直接使用的落地结构”，这一节就是“后续可能扩展到的完整分层”。

下面是目标闭环结构，用于说明后续可能扩展到哪里；不代表当前仓库已完整实现。
...
因为当前项目仍处于**极简规则阶段**。现阶段先验证极简层的稳定性、审计性和真实使用收益；更复杂的层级属于未来演进方向，不是当前运行前提。

### 4. 是否适合多个仓库复用？
适合。这也是本仓库的重要目标之一：让规则可以被版本化、迁移和复用。
...
本项目不会为了“体系完整”而提前引入全部层级。后续是否扩展完整规约、决策层、披露层和串联协议，将主要依据以下信号逐步推进：
...
也就是说，本项目更重视实用性、稳定性、可回退性和长期可维护性，而不是规则数量本身。
